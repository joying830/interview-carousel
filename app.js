...
const API_URL = 'https://script.google.com/macros/s/AKfycbwLLfVYIl_lfs5dh7OBJEpJHpr8ju1mrI-IPtEBV79RrxgVTlyCwBVzJ3eXhRcrBB9iow/exec';
...
async function loadData() {
  try {
    ...
    const { title, slides } = await res.json();
...
function renderSlides(slides) {
  ...
  slides.forEach((slide, i) => {
    ...
    slide.columnsData.forEach(col => {
      ...
      // 判斷 error / staticMessage / data / empty
      let inner = `<div class="column-title">${col.title}</div><div class="column-content">`;
